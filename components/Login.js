...
export default function Login() {
    ...
    return (
        <div className='flex flex-col flex-1 justify-center items-center gap-4'>
            ...
            <p className='text-center'>{isRegister ? 'Already have an account? ' : 'Don\'t have an account? '}<button onClick={() => setIsRegister(!isRegister)} className='text-indigo-600'>{isRegister ? 'Sign in' : 'Sign up'}</button>
            </p>
        </div>
    )
}
